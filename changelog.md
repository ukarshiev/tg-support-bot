0.36.26 – 30.08.2026
- [Антишум Telegram] `MESSAGE_TO_EDIT_NOT_FOUND` обрабатывается как штатный no-op на уровне общих Telegram-job и простых запросов: информационный лог без исключения, повторов и записи в `failed_jobs`.
- [Безопасная canary-проверка] Ночная `telegram:support-flow-check` больше не подставляет выдуманный `message_id` и не ставит заведомо невыполнимый `editMessageReplyMarkup`; уникальный `callback_id` сохраняет изоляцию языковых шагов и защиту от повторного нажатия.
- [Проверка] Добавлены регрессии no-op без retry/fail, отсутствия edit-запроса в служебном flow и callback-lock при нулевом `message_id`. Тесты не запускались по прямому ограничению задачи.

0.36.25 – 30.08.2026
- [Устойчивость сборки] Composer и npm получают до четырёх попыток с нарастающими паузами 5/15/30 секунд; после исчерпания попыток сборка завершается ошибкой.
- [Composer fallback] В build-слой Composer добавлены `git` и сертификаты, поэтому после сбоя dist-загрузки доступен штатный fallback на source; runtime-образ не изменён.
- [Медленный канал] Таймауты Composer увеличены до 600 секунд для HTTP-загрузок и внешних процессов. Docker-сборка и тесты не запускались по прямому ограничению задачи.

0.36.24 – 29.08.2026
- [Критический фикс релиза] Окна ожидания готовности рассчитаны из `start_period` и трёх интервалов Docker healthcheck с дополнительным запасом: 150 секунд для ядра и 180 секунд для Telegram poller.
- [Диагностика] Во время ожидания каждые 30 секунд выводится прогресс, а сообщения об ошибке показывают фактический таймаут из именованной переменной вместо зашитых 60 секунд.
- [Проверка] Добавлена unit-регрессия минимального 150-секундного окна poller и отсутствия зашитого числа в ошибке. Тесты не запускались по прямому ограничению задачи.

0.36.23 – 29.08.2026
- [Сеть Telegram] Добавлена необязательная настройка `TELEGRAM_PROXY` для HTTP- и SOCKS5h-прокси; пустое значение сохраняет прямой выход без изменения прежнего поведения.
- [Изоляция] Через прокси идут только запросы к `api.telegram.org`, включая файлы, callback-ответы и transport poller; внутренние webhook `http://nginx/...`, БД, Redis и Docker-сервисы остаются на прямом внутреннем маршруте.
- [Docker DNS] Для `app`, `queue`, `scheduler`, `telegram_poller` и `ai_telegram_poller` добавлен `host.docker.internal:host-gateway`, чтобы прокси хоста разрешался при пользовательских DNS.
- [Безопасность логов] Credentials в HTTP/SOCKS5h URL маскируются; добавлены регрессии прямого режима, обоих форматов прокси и обхода прокси внутренним webhook. Тесты не запускались по прямому ограничению задачи.

0.36.22 – 29.08.2026
- [Безопасный запуск ingress] `telegram_poller` и `ai_telegram_poller` теперь запускаются только после готовности базовых сервисов, успешного `artisan about` и статуса Horizon `running`; затем их health подтверждается отдельным циклом.
- [Таймаут миграции] Только для процесса `migrate --force` заданы PostgreSQL `lock_timeout=15s` и `statement_timeout=10min`, чтобы блокировка завершала релиз ошибкой и запускала штатный rollback, не меняя обычные подключения приложения.
- [Проверка] Добавлены unit-регрессии порядка запуска poller и ограничений миграционной сессии. Тесты не запускались по прямому ограничению задачи.

0.36.21 – 29.08.2026
- [Логи] Основной `stack` по умолчанию переведён с неротируемого `single` на суточный `daily`; срок хранения каналов `daily` и `app` сокращён с 14 до 7 дней.
- [Автоочистка] Добавлена ежедневная команда `logs:prune --days=7` в 03:30: она удаляет старые файлы непосредственно из `storage/logs`, сохраняет `.gitignore`, не обходит каталоги и поддерживает безопасный `--dry-run` с подсчётом файлов и байт.
- [Проверка] Добавлены регрессии удаления старого файла, сохранения свежего файла, `.gitignore` и вложенного каталога, сухого прогона и конфигурационных дефолтов. Тесты не запускались по прямому ограничению задачи.

0.36.20 – 29.08.2026
- [Безопасный релиз] `start.sh` после подтверждённого дампа и сохранения образов сначала собирает новый код, затем останавливает оба Telegram poller, очередь и scheduler; новый `app` и миграции запускаются только после остановки приёма и обработки.
- [Rollback] При ошибке после начала паузы rollback возвращает старые образы и принудительно поднимает оба poller, `queue` и `scheduler`, в том числе при сбое до миграции.
- [Эксплуатация] Перед паузой выводится предупреждение о 24-часовом хранении Telegram updates; порядок выкатки закреплён unit-регрессиями и runbook. Тесты не запускались по прямому ограничению задачи.

0.36.19 – 29.08.2026
- [Защита Telegram ingress] Внутренние webhook-ответы `502`, `503`, `504` и прочие недетерминированные HTTP-отказы теперь ведут себя как транспортный сбой: не расходуют лимит отравленного апдейта и не сдвигают poller offset; лимит расходуют только `4xx` и ровно `500`.
- [Durable quarantine] Перед пропуском после трёх детерминированных отказов полный Telegram update, HTTP-статус, число попыток и время сохраняются в новой таблице `discarded_telegram_updates`; добавлена обратимая миграция и модель.
- [Видимость входящих] Окончательный провал зеркала входящего Telegram/MAX/VK-сообщения в `telegram-support-topic` один раз ставит отдельное служебное уведомление в уже существующую тему с временем и типом вложения; уведомление не создаёт тему и не порождает рекурсию.
- [Самовосстановление] `delivery:reconcile-admin-replies` теперь сверяет также зависшие операции зеркалирования в support-topic и запускает тот же идемпотентный сигнал оператору.
- [Проверка] Добавлены регрессии `502`, `500`, durable-сохранения, `up/down/up` миграции, постановки и нерекурсивности уведомления, его отдельного текста и сверки зависшего зеркала. Тесты и миграция не запускались по прямому ограничению задачи.

0.36.18 – 29.08.2026
- [Надёжность Telegram canary] Ночная `telegram:support-flow-check` без отдельного списка теперь проверяет все включённые Telegram-языки; `--languages` и `telegram.health_check_languages` сохраняют приоритет.
- [Ожидание доставки] Убран фиксированный таймаут 10 секунд: дефолт 92 секунды учитывает пять сетевых попыток, backoff `2+5+10+20` и запас очереди; значение переопределяется опцией или настройкой.
- [Защита прогона] Добавлены пауза 1100 мс между языками, рассчитанный по числу шагов общий дедлайн и Cache-lock; параллельный запуск штатно пропускается, а не дублирует сообщения.
- [Отчёт] Успешные шаги агрегируются, ошибки остаются подробными, непроверенные языки явно перечисляются; длинный отчёт безопасно делится на сообщения не более 4096 символов.
- [Проверка] Добавлены регрессии дефолтного полного списка, явного override, живого lock, Telegram-лимита отчёта и языков, пропущенных по дедлайну. Тесты не запускались по прямому ограничению задачи.

0.36.17 – 29.08.2026
- [Доставка ответов] Исходящий ответ оператора сохраняется со статусом «в очереди»; `messages.delivery_status` синхронизируется с канонической `delivery_operations` при подтверждённой доставке и окончательном провале.
- [Оповещение] Окончательная недоставка один раз сообщает причину в уже существующей Telegram-теме; тема ради уведомления не создаётся, дедупликация выполняется по `operation_key`.
- [Самовосстановление] Добавлена команда `delivery:reconcile-admin-replies --minutes=15`, которая завершает зависшие `processing`-операции как failed и запускает то же уведомление.
- [Интерфейс] `/admin/chats` показывает «В очереди» / «Доставлено» / «Не доставлено», причину ошибки и предупреждение до отправки недоступному получателю; toast честно сообщает только о постановке в очередь.
- [Инвариант] AI-черновики по-прежнему хранятся только в `ai_messages`; новый механизм не создаёт для них строки в `messages`.
- [Проверка] Добавлены тесты начального статуса, обоих переходов, идемпотентности уведомления, сверки и веб-рендера. Тесты не запускались по прямому ограничению задачи.

0.36.16 – 29.08.2026
- [Критический фикс] Telegram HTTP 403 теперь классифицируется по нормализованным подстрокам полного `description`: недоступностью клиента считаются только `blocked by the user` и `user is deactivated`; прочие и неизвестные 403 логируются без ложной плашки.
- [Антимусор] Плашка «Пользователь заблокировал бота» отправляется только в уже существующий `topic_id`; новая forum-тема ради служебной плашки не создаётся.
- [Состояние диалога] В `bot_users` добавлены флаг, причина и время недоступности; состояние атомарно очищается при успешной Telegram-доставке или новом private-сообщении клиента и показывается оператору в карточке диалога.
- [Проверка] Тесты классификации всех известных 403, отсутствующей темы, установки/сброса состояния и веб-бейджа перенесены в зеркальные каталоги. Тесты и миграция подготовлены, но не запускались по ограничению задачи.

0.36.15 – 29.08.2026
- [Критический фикс] Telegram private-сообщения клиента теперь сохраняются и зеркалятся до выбора языка; служебные `/start` и `/lang` по-прежнему не создают обращение.
- [Антиспам] Автоматический селектор языка ограничен одной отправкой в минуту на `bot_user` через атомарный `Cache::add`; явные команды смены языка не ограничены.
- [Проверка] Добавлены регрессии сохранения/зеркалирования до выбора языка, антиспама, отсрочки для `/start`/`/lang` и сохранения идемпотентности. Тесты подготовлены, но не запускались по ограничению задачи.

0.36.14 – 29.08.2026
- [Критический фикс] Telegram poller ограничивает повторы отвергнутого внутренним webhook апдейта тремя попытками, после чего помечает его как отравленный, сдвигает offset и продолжает пачку; транспортный сбой webhook по-прежнему не сдвигает offset.
- [Надёжность] Редактирование отсутствующего Telegram-сообщения и неизвестные типы апдейтов теперь завершаются штатно с warning-логом вместо HTTP 500.
- [Проверка] Добавлены регрессии пропуска отравленного апдейта, сохранения offset при транспортной ошибке и безопасного редактирования отсутствующего сообщения. Тесты подготовлены, но не запускались по ограничению задачи.
...
0.36.10 – 31.07.2026
- [Фикс] (Plane TGSUPBOT-65 / Linear ID: KAR-327) Telegram onboarding — `/start`, `/lang`, выбор языка и приветствие больше не создают тему поддержки, не зеркалируются операторам и не отображаются как обращение в Web.
- [Фикс] (Plane TGSUPBOT-65 / Linear ID: KAR-327) Первое обращение — после первого реального текста или медиа Telegram-тема получает сначала «Контактную информацию», затем сообщение клиента; повторные сообщения существующих диалогов идут сразу.
- [Фикс] (Plane TGSUPBOT-65 / Linear ID: KAR-327) Web workspace — Telegram-диалог появляется только при наличии реального входящего сообщения, а виртуальная карточка контакта всегда располагается перед видимой перепиской.
- [Проверка] (Plane TGSUPBOT-65 / Linear ID: KAR-327) Изолированный PHPUnit — 154 теста и 423 assertions для Telegram ingress, порядка jobs, выбора языка и Web workspace.

0.36.9 – 30.07.2026
- [Завершение миграции] (Plane TGSUPBOT-80) Удалены неактивные дубли языка `chat_translation_locale*`; единственным источником языка клиента окончательно остаются `preferred_language_*`.
- [Безопасный rollback] (Plane TGSUPBOT-80) Откат миграции восстанавливает legacy-колонки и заполняет их из канонического языка, поэтому возврат предыдущего image не оставляет клиента без языка.

0.36.8 – 30.07.2026
- [Надёжность] (Plane TGSUPBOT-80) Production release — nginx выбирает TLS-шаблон только при наличии сертификатов, иначе использует Windows Docker-шаблон; новый конфиг гарантированно загружается через recreate.
- [Безопасный rollback] (Plane TGSUPBOT-80) Production release — при сбое возвращаются не только предыдущие application images, но и nginx-конфигурация; успех подтверждается health всех девяти сервисов.

0.36.7 – 30.07.2026
- [Критический фикс] (Plane TGSUPBOT-80) Production release — удалён вызов уже отсутствующего `assets_init`; nginx-конфигурация теперь безопасно создаётся из шаблона до переключения контейнеров.

0.36.6 – 30.07.2026
- [Критический фикс] (Plane TGSUPBOT-80) Язык клиента унифицирован: Telegram/VK/MAX и dropdown в `/admin/chats` теперь меняют одни поля `preferred_language_*`; миграция переносит в них самый свежий старый выбор. Legacy-колонки остаются неактивными на один rollback-совместимый релиз.
- [Фикс] (Plane TGSUPBOT-80) `Не выбран` доступен оператору как обычный выбор. Для него и для русского языка ручные, AI и системные ответы отправляют оригинал без машинного перевода; иностранные языки по-прежнему требуют готовый перевод.
- [Тесты] (Plane TGSUPBOT-80) Добавлены регрессии единого выбора языка, очистки языка, отправки оригинала, AI/автоответов и безопасного переноса старых данных.
...
- [Надёжность CI] (Plane TGSUPBOT-81) SQLite — TgMessage-тест явно связывает payload с созданным пользователем и не создаёт случайно второго пользователя на границе системной секунды.
- [Надёжность CI] (Plane TGSUPBOT-81) Docker — новое правило Hadolint DL3066 получило точечное документированное исключение для `www-data`, наличие которого гарантирует официальный PHP image и проверяет runtime gate.
