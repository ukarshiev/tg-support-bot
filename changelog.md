0.36.32 – 05.09.2026
- [Критическая идемпотентность Telegram] Многочастная отправка сохраняет подтверждение каждой части в общем Cache и при повторе продолжает с места сбоя; уже доставленный текст или медиа повторно не отправляются.
- [Гонка forum-темы] Ожидание AI-черновика атомарно ставит `TopicCreateJob` не более одного раза, а последующие попытки только проверяют появление `topic_id`.
- [Идемпотентность AI] Пара `(bot_user_id, source_hash)` закреплена уникальным индексом; повтор `SendAiDraftJob` не создаёт второй черновик, повторную доставку и второй SLA-alert.
- [Проверяемый rollback] `start.sh` заранее сообщает точный отсутствующий rollback-image, после восстановления проверяет health ядра и poller и при неуспехе требует ручного вмешательства с отдельным exit code.
- [Щадящий quarantine] Попытки внутреннего webhook разнесены на 0/5/20 секунд: краткий сбой приложения не теряет живой update, устойчиво битый update по-прежнему блокирует очередь только до третьего отказа.
- [Регрессии] Добавлены проверки продолжения многочастной отправки, единственной постановки создания темы, дедупликации AI-черновика, явного провала rollback и восстановления update до quarantine. Тесты не запускались по прямому ограничению задачи.

0.36.31 – 05.09.2026
- [Центральный лимит Telegram] Все `sendMessage`/`editMessageText` и медиа-подписи теперь проходят через единый заслон в `TelegramMethods`: текст свыше 4096 символов делится без потери содержимого, а подпись свыше 1024 усекается с явным маркером и полностью отправляется следующим сообщением.
- [Безопасная разметка] Разбиение идёт по строкам и словам; HTML-теги и MarkdownV2-форматирование закрываются и переоткрываются между частями, HTML-сущности и escape-последовательности не режутся. Локальные делители ночного отчёта и AI-черновика удалены.
- [Диагностика] `Bad Request: message is too long` распознаётся как `MESSAGE_TOO_LONG`, журналируется с методом и фактической длиной и считается детерминированным отказом без повторов; обработка `429`, `403` и исчезнувших topic не изменена.
- [Регрессии] Добавлены сценарии точной границы, полной многочастной доставки, валидности HTML/MarkdownV2, длинной media caption и детерминированной ошибки. Тесты не запускались по прямому ограничению задачи.

0.36.30 – 05.09.2026
- [Критический фикс AI-черновиков] Сгенерированный ответ теперь сначала сохраняется в `ai_messages` со статусом `pending`, а Telegram-доставка выполняется отдельной существующей `SendPendingAiDraftToTelegramJob`; ошибка Telegram больше не скрывает черновик от оператора и не создаёт запись в `messages`.
- [Лимит Telegram] Двуязычный черновик безопасно делится на сообщения не длиннее 4096 символов: HTML-теги и сущности не разрываются, весь текст сохраняется, inline-кнопки ставятся на последнюю часть, а её `message_id` остаётся ключом принятия и отмены.
- [Пустой ответ AI] `null` и пустой текст провайдера явно журналируются с пользователем, платформой и провайдером; пустой `ai_messages` не создаётся, попытка сохраняется в операционном логе.
- [Регрессии] Добавлены сценарии сохранения при ошибке Telegram, полной многочастной доставки, кнопок и принятия по последнему `message_id`, пустого ответа провайдера и запрета записи черновика в `messages`. Тесты не запускались по прямому ограничению задачи.

0.36.29 – 05.09.2026
- [Надёжность доставки] Политика повторов Telegram-вызовов клиенту вынесена в единый `TelegramClientDeliveryRetryPolicy`: 9 попыток с растущими паузами `5/10/20/40/80/160/240/300` дают окно 15 минут 27 секунд вместо примерно минуты и перекрывают измеренный пятиминутный обрыв.
- [Ночная проверка] `telegram:support-flow-check` использует отдельный 150-секундный canary-лимит из общей политики и общий потолок прогона 3600 секунд. Неподтверждённый вовремя шаг теперь отличается от окончательного провала доставки; оставшиеся после дедлайна языки по-прежнему отмечаются как непроверенные.
- [Совместимость] `429` по-прежнему использует `retry_after`; детерминированные `403/400` не повторяются; уведомление менеджеру о временной недоставке ставится только после исчерпания всех попыток.
- [Регрессии] Добавлены сценарии растущих пауз, успешной доставки после временного сбоя, окончательного провала с уведомлением, отсутствия retry для `403`, соблюдения `retry_after` и общего источника таймаута ночной проверки. Тесты не запускались по прямому ограничению задачи.

0.36.28 – 04.09.2026
- [Гигиена тестов] Идентификаторы payload-моков VK, Telegram, MAX и External переведены с `time()` на общий детерминированный счётчик; временные поля используют одну метку на payload.
- [Регрессия] `VkEditServiceTest` и связанные Telegram-тесты теперь берут связь с пользователем из DTO/мока, поэтому переход через границу секунды больше не меняет тестового пользователя. Тесты не запускались по прямому ограничению задачи.

0.36.27 – 02.09.2026
- [Безопасность переписки] Любая команда менеджера, начинающаяся с `/`, перехватывается внутри forum-топика до клиентской доставки; неизвестная команда остаётся в топике и показывает список реально доступных команд.
- [Управление контактом] Добавлены `/ban`, `/unban` и Telegram-формы с упоминанием бота (`/ban@bot`): команды используют тот же `BannedContactMessage`, что и inline-кнопки, и явно подтверждают блокировку или разблокировку в карточке контакта.
- [Регрессии] Добавлены сценарии ban/unban, bot mention, неизвестной команды, обычного ответа менеджера и неизменного private `/start`. Тесты не запускались по прямому ограничению задачи.
