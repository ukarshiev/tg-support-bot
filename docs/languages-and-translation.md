...
Перед отправкой текста в машинный перевод система временно заменяет переменные, ссылки и упоминания непрозрачными ASCII-маркерами вида `__TGSPH_<сигнатура>_0000__`. Сам ключ `{{connector}}` и значение ссылки вообще не попадают в запрос переводчика, поэтому Yandex, Google и offline-провайдер не могут их перевести. ASCII выбран по результатам боевой проверки: Yandex и Google для части языков удаляли одиночные Unicode PUA-маркеры.

После ответа система работает fail-closed:

- каждый marker должен вернуться ровно один раз;
- после восстановления набор переменных, ссылок и упоминаний обязан совпасть с источником;
- переводчик не должен добавить HTML/XML-разметку;
- повреждённый ответ не получает статус `ready`, не попадает в кэш и передаётся следующему провайдеру;
- старый сохранённый перевод с переведёнными ключами или остатками `<x>` не используется для системного автоответа.
...
sequenceDiagram
    ...
    Guard->>MT: Текст с непрозрачным marker
    MT-->>Guard: Перевод с тем же marker
    Guard->>Guard: Проверяет round-trip
...
Язык клиента единственный для всей системы:

- клиент выбирает его в Telegram/VK/MAX, оператор может изменить тот же язык в верхней панели `/admin/chats`;
- источник истины — `bot_users.preferred_language_code`;
- значение `Не выбран` очищает язык и доступно оператору как обычный пункт списка;
- для `Не выбран` и `ru` клиенту отправляется оригинал без вызова машинного перевода;
- для остальных языков перевод перед отправкой остаётся обязательным.

Медиа не переводятся, но должны идти в той же хронологии, что и текст.

История диалога тоже переводится:

- язык истории берётся из единственного `bot_users.preferred_language_code`;
- отдельного языка текущего чата больше нет;
- для `ru` переводчик показывает флаг, но jobs не создаёт;
- для `Не выбран` перевод истории и исходящего сообщения не запускается;
- для нерусского языка `ConversationPage` создаёт записи `message_translations` со статусом `queued`;
- видимая история переводится пачкой через `TranslateMessageHistoryBatchJob`;
- ручной retry одного failed-сообщения переводится через `TranslateMessageHistoryJob`;
- jobs обновляют `translation_jobs`: `queued → running → done/failed/skipped`;
...
- failed-переводы не перезапускаются автоматически при открытии чата или скролле: только кнопкой `Повторить`.
- один batch режется до 25 текстов и до 5000 символов, чтобы не упираться в задержки/лимиты провайдеров.
...
1) `.\scripts\run-isolated-tests.ps1 tests/Unit/Livewire/Chat/ConversationPageTest.php` — Почему: проверяет перевод истории, dropdown языка, retry и старые регрессии чата.
2) `.\scripts\run-isolated-tests.ps1 tests/Unit/Modules/Translation tests/Unit/Jobs/TranslateMessageHistoryBatchJobTest.php` — Почему: проверяет batch-перевод, кэш, лимиты пачек и обновление статусов.
3) `.\scripts\run-isolated-tests.ps1 tests/Feature/Settings/TranslationQueuePageTest.php` — Почему: проверяет страницу очереди, где видны задачи истории и support-кейсов.
4) `.\scripts\run-isolated-tests.ps1 tests/Feature/Settings/AiKnowledgePageTest.php tests/Unit/Modules/Ai/Support/AiSupportRagTest.php` — Почему: проверяет RU canonical, защиту ручных правок, UI support-кейса и гибридный RAG.
5) `.\scripts\run-isolated-tests.ps1 tests/Unit/Modules/Telegram/Api/ParserMethodsTest.php tests/Unit/Jobs/SendMessage/AbstractSendMessageJobTest.php tests/Unit/Modules/Telegram/Actions/SendStartMessageTest.php tests/Unit/Modules/Telegram/Actions/AnswerCallbackQueryTest.php tests/Feature/Modules/Telegram/IncomingMessagePersistenceTest.php --filter="language|callback|full_start|post_query|get_query|attach_query|transient"` — Почему: проверяет Telegram UI выбора языка, тихий callback, timeout HTTP-клиента, retry временных ошибок, антидубль быстрых welcome-кликов, отправку welcome и отсутствие повторной контактной карточки.
6) Playwright открыть `/admin/chats`, `/admin/settings/ai/knowledge` и `/admin/settings/language/translate_queue` — Почему: убедиться, что Livewire UI реально кликается без 500.


## Что сделать, чтобы применить изменения:

1) `docker compose build app queue scheduler telegram_poller ai_telegram_poller && docker compose up -d app queue scheduler telegram_poller ai_telegram_poller` — Почему: PHP-код и queue-jobs запечены в Docker-образ, поэтому после правок страницы/сервисов нужна пересборка контейнеров.
