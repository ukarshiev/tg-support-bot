...
Проверка запускается планировщиком каждые 3 часа через `routes/console.php`:
...
2) `docker compose exec -T app php artisan migrate --force` — Почему: создать системные шаблоны без перезаписи ручных переводов.
...
- после выбора языка клиенту отдельным сообщением уходит приветствие на выбранном языке;
...
- каждое новое нажатие кнопки языка отправляет новое приветствие, даже если такое же приветствие уже было в истории диалога;
...
- повторная доставка Telegram одного и того же callback не создаёт дубль: защита строится по уникальному `callback_query.id`; разные осознанные нажатия не блокируются;
