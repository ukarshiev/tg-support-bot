...
### Ночной canary всех языков

Команда `telegram:support-flow-check` проверяет `/start`, `/lang` и welcome каждого включённого Telegram-языка. Если `--languages` не указан и `telegram.health_check_languages` пуст, список автоматически берётся из текущих включённых языков; поэтому новый язык не останется вне ночной проверки.

Приоритет списка: `--languages` → непустой `telegram.health_check_languages` → все включённые языки. Между языками по умолчанию выдерживается 1100 мс, а подтверждение доставки ожидается до 92 секунд. Общий дедлайн рассчитывается из числа selector/language-проверок, пауз и 60 секунд запаса на отчёт; для 24 языков это 2478 секунд. Значения можно переопределить соответственно через `--language-pause`, `--await-timeout`, `--deadline` или одноимённые настройки `telegram.health_check_*`.

Если общий дедлайн закончился, команда не начинает новые языки и перечисляет их в служебном отчёте как непроверенные. Это отличается от реальной ошибки уже начатой проверки. Успехи сворачиваются в одну строку, ошибки остаются подробными, а отчёт длиннее лимита Telegram делится на несколько сообщений.
