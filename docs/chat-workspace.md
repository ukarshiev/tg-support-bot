...
- Новый ответ сразу виден оператору как «В очереди», но не считается доставленным.
- `delivery_operations` остаётся источником истины: после успешной клиентской job пузырь получает «Доставлено», после исчерпания повторов — «Не доставлено» и причину из `last_error`.
...
- Окончательная ошибка один раз публикуется служебным сообщением в существующей support-теме. При отсутствии `topic_id` тема не создаётся.
- Если `bot_users.is_unavailable = true`, над формой видна причина и предупреждение; повторная попытка остаётся доступной.
- Toast после нажатия подтверждает только постановку в очередь, а не доставку клиенту.
- AI-черновики не участвуют в этом потоке и остаются только в `ai_messages` до реальной отправки клиенту.
