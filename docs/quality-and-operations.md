...
4. Поднять `pgdb`, `redis`, новый `app`, очистить `bootstrap/cache` и выполнить `migrate --force`.
5. Выполнить preflight, очистку/сборку Laravel-кэшей, затем поднять `queue`, `reverb`, `scheduler`.
6. Пересоздать `nginx`, поднять оба poller и проверить здоровье всех сервисов.

Пауза безопасна для входящих сообщений: Telegram хранит недоставленные updates до 24 часов, а poller обработают накопленное после запуска. Остановка старой очереди до нового `app` не даёт старым worker получить задачи классов, существующих только в новом образе. Если после начала паузы любой шаг падает, rollback возвращает прежние образы и поднимает оба poller, `queue` и `scheduler` даже до запуска миграции.
