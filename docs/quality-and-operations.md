...
## Предрелизная устойчивость повторов и rollback

- Длинная Telegram-отправка сохраняет checkpoint после каждой успешной части в общем Cache на 24 часа (окно больше всех штатных retry). При повторе с тем же ключом уже подтверждённые части не уходят повторно; AI-черновик использует стабильный ключ по `ai_messages.id`, обычные Telegram-job сохраняют уникальный ключ в сериализованном payload самой job.
- Если forum-тема AI-черновика ещё не создана, атомарный marker разрешает поставить `TopicCreateJob` только первой попытке. Остальные попытки только ждут `topic_id`; marker снимается после успешного создания темы.
- `ai_messages` защищены уникальным индексом `(bot_user_id, source_hash)`. Повтор генерации того же нормализованного ответа не создаёт второй черновик, повторную Telegram-доставку и второй SLA-alert.
- Детерминированные отказы внутреннего webhook основного poller повторяются через 5 и 15 секунд: фактические попытки приходятся примерно на 0, 5 и 20 секунды. Третий отказ по-прежнему переносит update в `discarded_telegram_updates` и освобождает очередь.
- Rollback проверяет наличие каждого сохранённого image до retag, затем ждёт фактического health ядра и, если они останавливались, обоих poller. Строка `AUTOMATIC ROLLBACK FAILED ... manual intervention is required` означает, что автоматическое восстановление не подтверждено и производство нужно восстанавливать вручную.
- Уникальный индекс AI добавляет миграция `2026_09_05_000001_make_ai_message_source_hash_unique_per_user.php`. На production её применять только штатным релизом после свежего дампа и явного подтверждения; старые дубли строк сохраняются, повторный `source_hash` у них очищается.

## Центральный заслон длины Telegram

- Единственная точка контроля — `TelegramMethods::sendQueryTelegram()`, непосредственно перед `ParserMethods::postQuery()`/`attachQuery()`. Новые отправки через штатный Telegram API-слой защищены автоматически.
- `sendMessage` и `editMessageText` ограничены 4096 символами. Длинный текст делится по строкам и словам; кнопки остаются только на последней части, а наружу возвращается её `message_id`.
- Для `sendPhoto`, `sendVideo`, `sendDocument`, `sendAudio`, `sendVoice` и других `send*` с полем `caption` действует лимит 1024. Подпись заканчивается маркером «подпись усечена; полный текст ниже», затем исходный полный текст отправляется через `sendMessage` в тот же чат/topic.
- HTML-теги закрываются в конце части и переоткрываются в следующей; сущности не режутся. MarkdownV2-разделители также балансируются, escape-последовательности остаются целыми. `TelegramMarkupSanitizer` не содержит парсера разметки — только перевод в plain text, поэтому переиспользуется как аварийная очистка прежнего error-flow, а центральный splitter является единственным парсером деления.
- Ответ `Bad Request: message is too long` классифицируется как `MESSAGE_TOO_LONG`. Это детерминированный отказ без retry; error-лог `telegram_message_too_long` содержит метод и фактическую длину, но не текст сообщения.
- `429` продолжает использовать Telegram `retry_after`, `403` не повторяется, `TOPIC_NOT_FOUND`/`TOPIC_DELETED`/`TOPIC_ID_INVALID` продолжают пересоздавать topic.
...
- Ошибка Telegram не удаляет и не отменяет черновик: оператор продолжает видеть его в `/admin/chats`. Временный сбой допускает повтор job для существующей записи, а детерминированный `400 Bad Request: message is too long` явно журналируется и не повторяется.
- Telegram-текст делится центральным заслоном на части до 4096 символов. Каждая часть содержит валидный HTML: открытые теги закрываются перед границей и переоткрываются после неё, HTML-сущности не режутся.
