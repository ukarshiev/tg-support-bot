# Последняя редакция: 29.08.2026 UTC+3

# Качество и эксплуатация

## Контроль зависших ответов оператора
...
- Служебное уведомление о недоставке использует отдельный детерминированный ключ `admin-reply-failure-notification:<operation_key>`, поэтому повторная обработка не создаёт дубль.
- Уведомление допускается только при уже существующем `topic_id`; `TopicCreateJob` из этого потока не запускается.
- Если worker был убит и `failed()` не выполнился, команда `php artisan delivery:reconcile-admin-replies --minutes=15` помечает старые `processing`-операции проваленными. `--minutes` должен быть не меньше 1.
...
- Для регулярного контроля отслеживать количество `admin-reply` в `processing` старше выбранного порога и ошибки `admin_reply_failure_notification_failed`.
...
- Ожидание одного подтверждения задаётся `--await-timeout` или `telegram.health_check_await_timeout_seconds`. Дефолт — 92 секунды: 5 сетевых попыток по 8 секунд, backoff `2+5+10+20` и 15 секунд запаса очереди.
...
- Общий предел задаётся `--deadline` или `telegram.health_check_deadline_seconds`. По умолчанию он рассчитывается как таймауты двух selector-проверок и всех языков + межъязыковые паузы + 60 секунд на завершение и отчёт (для 24 языков при штатных значениях — 2478 секунд). После дедлайна новые шаги не запускаются, а оставшиеся языки попадают в отчёт как непроверенные.
- Cache-lock `telegram:support-flow-check:lock` не допускает двух прогонов одновременно. Повторный запуск при живом lock завершается успешно с сообщением `another run is still active`.
...
- Основной Telegram poller различает отказ приложения и транспорта: HTTP 4xx/5xx внутреннего webhook повторяется не более трёх раз для одного `update_id`, затем апдейт логируется как `telegram_poller_poisoned_update`, offset сдвигается, а обработка пачки продолжается.
- Ошибка соединения или timeout внутреннего webhook не расходует лимит отравленного апдейта и не сдвигает offset: доставка будет повторена после восстановления транспорта.
- Для контроля head-of-line blocking отслеживать `telegram_poller_poisoned_update`, `update_id`, HTTP `status` и число `attempt`; рост таких событий означает устойчивую ошибку обработчика, но очередь клиентских сообщений не должна останавливаться.
