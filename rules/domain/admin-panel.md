...
**BR-003f** — An admin reply is created with `messages.delivery_status = pending`. Its `admin-reply` row in `delivery_operations` is the source of truth: the delivery job starts `processing`, confirmed client delivery synchronizes both rows to `delivered`, and a terminal error synchronizes both to `failed` while preserving the reason in `delivery_operations.last_error`. The chat bubble must render all three states and the failure reason. The post-submit toast may confirm only queueing, never delivery.

**BR-003g** — A terminal admin-reply failure queues at most one service notification per original `operation_key`, and only when the `BotUser` already has a `topic_id`; this failure flow must never create a forum topic. `delivery:reconcile-admin-replies --minutes=<N>` applies the same terminal transition and notification to stale `processing` operations. AI drafts remain exclusively in `ai_messages`; no delivery-status flow may persist a draft into `messages` before an actual client send.

**BR-003h** — When the active `BotUser.is_unavailable` is true, the reply form must show the stored reason before the operator sends. The form remains enabled so the operator can deliberately retry.
